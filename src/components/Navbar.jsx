    ...
    <nav className='w-full flex py-6 justify-between items-center navbar'>
      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`font-poppins font-normal cursor-pointer text-base ${index === navLinks.length - 1 ? "mr-0" : "mr-10"} text-white hover:text-secondary`}
          >
          ...
          <div className={`${ toggle ? 'flex' : 'hidden'} p-6 bg-black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[200px] rounded-xl sidebar`}>
            <ul className="list-none flex flex-col justify-end items-center flex-1">
              {navLinks.map((nav, index) => (
                <li
                  key={nav.id}
                  className={`font-poppins font-normal cursor-pointer text-base ${index === navLinks.length - 1 ? "mb-0" : "mb-4"} text-white hover:text-secondary`}
                >
                  <a href={`#${nav.id}`}>{nav.title}</a>
                </li>
              ))}
